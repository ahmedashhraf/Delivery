...
public abstract class BaseLayer implements DrawingContent, AnimationListener, KeyPathElement {
    ...
    static /* synthetic */ class C28462 {
        ...
        static {
            ...
            L_0x007d:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.model.layer.BaseLayer.C28462.<clinit>():void");
        }
    }
    ...
    private void applyMasks(Canvas canvas, Matrix matrix2) {
        ...
        for (int i = 0; i < this.mask.getMasks().size(); i++) {
            ...
            int i2 = C28462.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask2.getMaskMode().ordinal()];
            if (i2 == 1) {
                if (i == 0) {
        ...
        C2801L.beginSection(str2);
        canvas.restore();
        C2801L.endSection(str2);
    }
    ...
    static BaseLayer forModel(Layer layer, LottieDrawable lottieDrawable2, LottieComposition lottieComposition) {
        switch (C28462.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void addAnimation(@C0195i0 BaseKeyframeAnimation<?, ?> baseKeyframeAnimation) {
    ...
    public void draw(Canvas canvas, Matrix matrix2, int i) {
        C2801L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C2801L.endSection(this.drawTraceName);
            return;
        }
        ...
        if (hasMatteOnThisLayer() || hasMasksOnThisLayer()) {
            ...
            C2801L.endSection(str3);
            if (!this.rect.isEmpty()) {
                String str4 = "Layer#saveLayer";
                C2801L.beginSection(str4);
                saveLayerCompat(canvas, this.rect, this.contentPaint, true);
                C2801L.endSection(str4);
                clearCanvas(canvas);
                C2801L.beginSection(str2);
                ...
                if (hasMatteOnThisLayer()) {
                    ...
                    C2801L.beginSection(str6);
                    C2801L.beginSection(str4);
                    saveLayerCompat(canvas, this.rect, this.mattePaint, false);
                    C2801L.endSection(str4);
                    clearCanvas(canvas);
                    this.matteLayer.draw(canvas, matrix2, intValue);
                    C2801L.beginSection(str5);
                    canvas.restore();
                    C2801L.endSection(str5);
                    C2801L.endSection(str6);
                }
                C2801L.beginSection(str5);
                canvas.restore();
                C2801L.endSection(str5);
            }
            recordRenderTime(C2801L.endSection(this.drawTraceName));
            return;
        }
        this.matrix.preConcat(this.transform.getMatrix());
        C2801L.beginSection(str2);
    ...
    /* access modifiers changed from: 0000 */
    public void setMatteLayer(@C0195i0 BaseLayer baseLayer) {
        this.matteLayer = baseLayer;
    }

    /* access modifiers changed from: 0000 */
    public void setParentLayer(@C0195i0 BaseLayer baseLayer) {
