...
public class ImagePipelineFactory {
    ...
    public static PlatformDecoder buildPlatformDecoder(PoolFactory poolFactory, boolean z) {
        ...
        if (i >= 21) {
            ...
            return new ArtDecoder(poolFactory.getBitmapPool(), flexByteArrayPoolMaxNumThreads, new C0943c(flexByteArrayPoolMaxNumThreads));
        } else if (!z || i >= 19) {
    ...
    public static synchronized void initialize(ImagePipelineConfig imagePipelineConfig) {
        synchronized (ImagePipelineFactory.class) {
            if (sInstance != null) {
                FLog.m15789w(TAG, "ImagePipelineFactory has already been initialized! `ImagePipelineFactory.initialize(...)` should only be called once to avoid unexpected behavior.");
            }
