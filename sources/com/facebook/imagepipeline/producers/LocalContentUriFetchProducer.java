...
public class LocalContentUriFetchProducer extends LocalFetchProducer {
    ...
    private static final String[] PROJECTION = {FeatureRequestEntry.COLUMN_ID, "_data"};
        ...
        if (UriUtil.isLocalContactUri(sourceUri)) {
            if (sourceUri.toString().endsWith("/photo")) {
                ...
                if (sourceUri.toString().endsWith("/display_photo")) {
                    try {
                        inputStream = this.mContentResolver.openAssetFileDescriptor(sourceUri, AckRequest.ELEMENT).createInputStream();
