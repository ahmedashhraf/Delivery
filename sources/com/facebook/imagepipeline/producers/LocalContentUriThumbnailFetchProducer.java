...
public class LocalContentUriThumbnailFetchProducer extends LocalFetchProducer implements ThumbnailProducer<EncodedImage> {
    ...
    static {
        String str = "_data";
        PROJECTION = new String[]{FeatureRequestEntry.COLUMN_ID, str};
    ...
    private EncodedImage getCameraImage(Uri uri, ResizeOptions resizeOptions) throws IOException {
        ...
        try {
            ...
            if (resizeOptions != null) {
                EncodedImage thumbnail = getThumbnail(resizeOptions, query.getInt(query.getColumnIndex(FeatureRequestEntry.COLUMN_ID)));
                if (thumbnail != null) {
    ...
    private static int getRotationAngle(String str) {
        if (str != null) {
            try {
                return JfifUtil.getAutoRotateAngleFromOrientation(new ExifInterface(str).getAttributeInt(C2108a.f8392y, 1));
            } catch (IOException e) {
                FLog.m15752e(TAG, (Throwable) e, "Unable to retrieve thumbnail rotation for %s", str);
            }
        }
        return 0;
    }
