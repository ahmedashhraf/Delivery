package p076c.p112d.p134b.p135a.p262f.p266b.p267d;

import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.io.StringReader;
import java.security.GeneralSecurityException;
import java.security.KeyStore;
import java.security.cert.Certificate;
import java.security.cert.CertificateException;
import java.util.ArrayList;
import java.util.List;
import javax.net.ssl.TrustManagerFactory;
import javax.net.ssl.X509TrustManager;
import p076c.p112d.p134b.p135a.p136d.p261m.C6303a;
import p076c.p112d.p134b.p135a.p136d.p261m.C6303a.C6304a;
import p076c.p112d.p134b.p135a.p136d.p261m.C6306b.C6308b;
import p076c.p112d.p134b.p135a.p143g.C2766f;
import p076c.p112d.p134b.p135a.p143g.C6371e;
import p076c.p112d.p134b.p135a.p143g.C6378g0;
import p076c.p112d.p134b.p135a.p143g.C6384i0;
import p076c.p112d.p134b.p135a.p143g.C6399n0;
import p076c.p112d.p134b.p135a.p143g.C6436w;

@C2766f
/* renamed from: c.d.b.a.f.b.d.a */
/* compiled from: TestCertificates */
public class C6349a {

    /* renamed from: a */
    public static final C6350a f17746a = new C6350a("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----");

    /* renamed from: b */
    public static final String f17747b = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----";

    /* renamed from: c */
    public static final C6350a f17748c = new C6350a("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----");

    /* renamed from: d */
    public static final String f17749d = "-----BEGIN PRIVATE KEY-----\\nE+VTo/KjRA8P2ogks9bltd6f\n-----END PRIVATE KEY-----";

    /* renamed from: e */
    public static final C6350a f17750e = new C6350a("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n");

    /* renamed from: f */
    public static final String f17751f = ".eyJmb28iOiJiYXIifQ.eWzIsJF4PExQap9HK6Vlz8DGlgGwoiLCtyOEK0Bfu_yHTAZeApn5rh6Uzfx06Gv6eHdM34YL_tgLRb4bjuZVA8xvQ9uHNs8UtpBIOiUcagzvtKyyfCofk5U5sNb54GgVVYxa6p4A1ObdJv1jjlUOnzR8keX5LsAM4Ia7xeqiFh0GER4l0ulVChy_bSn0IeNiKFW7HKcxtcGO_zZTtlv4HiifuyPSk_ar2IDX1w599KXniVcWkQ_W1zcp5YuPDw8mIQDVCH2uQY7qs2ejdZj5LIgIz4CbQ0wg53rlwE7DDQM6MNUgZLnzNmMSMfFrpE7_PQyxe2qJCsucHODzEHX4Tg";

    /* renamed from: g */
    private static C6303a f17752g = null;

    @C2766f
    /* renamed from: c.d.b.a.f.b.d.a$a */
    /* compiled from: TestCertificates */
    public static class C6350a {

        /* renamed from: a */
        private String f17753a;

        public C6350a(String str) {
            this.f17753a = str;
        }

        /* renamed from: a */
        public String mo25694a() throws IOException {
            return C6371e.m29643c(mo25696c());
        }

        /* renamed from: b */
        public Certificate mo25695b() throws IOException, CertificateException {
            return C6384i0.m29688g().generateCertificate(new ByteArrayInputStream(mo25696c()));
        }

        /* renamed from: c */
        public byte[] mo25696c() throws IOException {
            return C6378g0.m29657a(new StringReader(this.f17753a), "CERTIFICATE").mo25757a();
        }

        /* renamed from: d */
        public X509TrustManager mo25697d() throws IOException, GeneralSecurityException {
            KeyStore instance = KeyStore.getInstance(KeyStore.getDefaultType());
            instance.load(null, null);
            instance.setCertificateEntry("ca", mo25695b());
            TrustManagerFactory instance2 = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm());
            instance2.init(instance);
            return (X509TrustManager) instance2.getTrustManagers()[0];
        }
    }

    /* renamed from: a */
    public static C6303a m29589a() throws IOException {
        if (f17752g == null) {
            C6304a aVar = new C6304a();
            aVar.mo25548c("RS256");
            ArrayList a = C6436w.m29853a();
            a.add(f17746a.mo25694a());
            a.add(f17748c.mo25694a());
            aVar.mo25547b((List<String>) a);
            C6308b bVar = new C6308b();
            bVar.m29355b("foo", (Object) "bar");
            String str = f17751f;
            int indexOf = str.indexOf(46, 2723);
            f17752g = new C6303a(aVar, bVar, C6371e.m29640a(str.substring(indexOf + 1)), C6399n0.m29727a(str.substring(0, indexOf)));
        }
        return f17752g;
    }
}
